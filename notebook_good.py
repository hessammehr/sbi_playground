...
def _(mo):
    mo.md("""
    # Milestone 5 — Bayesian self-consistency on unlabelled real patches

    This section uses the Bayesian log-ratio variance objective on unlabelled real image patches. The real-image objective is **not** reconstruction MSE and does not train a renderer. Latent proposals are sampled from a stop-gradient/frozen copy of the current NumPyro guide, and the guide is updated only through `log q_phi(theta | x_real)` in

    \[
    r(	heta, x) = \log p_{model}(	heta, x) - \log q_\phi(	heta \mid x).
    \]
    """)
    return
...
@app.cell
def _(IMAGE_SHAPE, jnp, np, plt):
    from pathlib import Path
    from PIL import Image

    REAL_IMAGE_PATH_M5 = Path("example.jpg")
    M5_REAL_PATCH_SIZE = IMAGE_SHAPE[0]
    M5_NUM_REAL_PATCHES = 8
    M5_PATCH_STRIDE = 64


    def load_real_patches_m5(
        path=REAL_IMAGE_PATH_M5,
        patch_size=M5_REAL_PATCH_SIZE,
        n_patches=M5_NUM_REAL_PATCHES,
        stride=M5_PATCH_STRIDE,
    ):
        """Deterministically extract high-contrast unlabelled patches from example.jpg.

        This fixed contrast normalisation is a preprocessing step, not a learned
        renderer or reconstruction target.
        """
        image = -np.asarray(Image.open(path).convert("L"), dtype=np.float32) / 255.0
        lo, hi = np.percentile(image, [1, 99])
        image_norm = np.clip((image - lo) / (hi - lo + 1e-6), 0.0, 1.0)
        height, width = image_norm.shape
        candidates = []
        for _y in range(0, height - patch_size + 1, stride):
            for _x in range(0, width - patch_size + 1, stride):
                _patch = image_norm[_y : _y + patch_size, _x : _x + patch_size]
                _score = float(np.std(_patch) * (1.0 - abs(np.mean(_patch) - 0.45)))
                candidates.append((_score, _y, _x))
        selected = sorted(candidates, reverse=True)[:n_patches]

        patches = []
        metadata = []
        for _score, _y, _x in selected:
            _patch = image_norm[_y : _y + patch_size, _x : _x + patch_size]
            _lo, _hi = np.percentile(_patch, [2, 98])
            _patch = np.clip((_patch - _lo) / (_hi - _lo + 1e-6), 0.0, 1.0)
            ...
            metadata.append(
                {
                    ...
                    "x": int(_x),
                    "score": float(_score),
                    "mean": float(_patch.mean()),
                    "std": float(_patch.std()),
                }
            )
        return jnp.asarray(np.stack(patches)), metadata


    real_images_m5, real_patch_metadata_m5 = load_real_patches_m5()
    real_patch_summary_m5 = {
        "shape": tuple(real_images_m5.shape),
        "min": float(jnp.min(real_images_m5)),
        "max": float(jnp.max(real_images_m5)),
        "mean": float(jnp.mean(real_images_m5)),
        "std": float(jnp.std(real_images_m5)),
    }

    fig_real_patches_m5, axes_real_patches_m5 = plt.subplots(2, 4, figsize=(9, 4.8))
    for _ax, _idx in zip(axes_real_patches_m5.ravel(), range(M5_NUM_REAL_PATCHES)):
        _ax.imshow(np.asarray(real_images_m5[_idx]), cmap="magma", interpolation="nearest")
        _meta = real_patch_metadata_m5[_idx]
        _ax.set_title(
            f"patch {_idx}\ny={_meta['y']}, x={_meta['x']}\nstd={_meta['std']:.2f}",
            fontsize=8,
        )
        _ax.set_xticks([])
        _ax.set_yticks([])
    fig_real_patches_m5.suptitle("Unlabelled real patches used for self-consistency", y=1.02)
    fig_real_patches_m5.tight_layout()
    fig_real_patches_m5
    return M5_NUM_REAL_PATCHES, real_images_m5, real_patch_summary_m5
...
def _(
    ...
    proposal_check_m5 = sample_guide_latents_m5(
        guide_params_m4, real_images_m5, M5_NUM_CHECK_PROPOSALS, random.PRNGKey(701)
    )
    log_joint_check_m5 = log_joint_samples_m5(real_images_m5, proposal_check_m5)
    log_q_check_m5 = guide_log_prob_samples_m5(
        guide_params_m4, real_images_m5, proposal_check_m5
    )
    sc_initial_check_loss_m5, sc_initial_grad_norm_m5, sc_initial_gradients_finite_m5 = sc_value_grad_norm_m5(
        guide_params_m4, real_images_m5, proposal_check_m5, log_joint_check_m5
    )
    ...
    mo.md(
        ...
        f"Real patch summary: `{real_patch_summary_m5}`.\n\n"
        f"Finite log p/log q/SC loss/gradient: `{sc_initial_finite_m5}`.\n\n"
...
def _(
    ...
    fixed_eval_samples_m5 = sample_guide_latents_m5(
        guide_params_m4, real_images_m5, M5_NUM_FIXED_EVAL_PROPOSALS, random.PRNGKey(740)
    )
    fixed_eval_log_joint_m5 = log_joint_samples_m5(real_images_m5, fixed_eval_samples_m5)
    fixed_eval_sc_before_m5 = float(
        sc_loss_m5(guide_params_m4, real_images_m5, fixed_eval_samples_m5, fixed_eval_log_joint_m5)
    )
    ...
    training_history_m5 = []
    training_key_m5 = random.PRNGKey(741)
    ...
    for step_m5 in range(M5_NUM_STEPS):
        ...
        proposals_m5 = sample_guide_latents_m5(
            guide_params_current_m5, real_images_m5, M5_NUM_PROPOSALS, subkey_m5
        )
        log_joint_proposals_m5 = log_joint_samples_m5(real_images_m5, proposals_m5)
    ...
    fixed_eval_sc_after_m5 = float(
        sc_loss_m5(guide_params_m5, real_images_m5, fixed_eval_samples_m5, fixed_eval_log_joint_m5)
    )
    sc_fixed_eval_decreased_m5 = fixed_eval_sc_after_m5 < fixed_eval_sc_before_m5

    mo.md(
        "### Milestone 5 conservative SC adaptation\n\n"
        "The update is intentionally small: larger real-image SC steps can distort the synthetic posterior, so the notebook uses gradient clipping and a tiny learning rate. Proposals are resampled each step from a stop-gradient copy of the current guide.\n\n"
...
def _(
    ...
    synthetic_summary_before_m5 = synthetic_ground_truth_summary_m5(
        guide_params_m4, synthetic_val_m4
    )
    synthetic_summary_after_m5 = synthetic_ground_truth_summary_m5(
    ...
    synthetic_no_collapse_m5 = bool(
        ...
        and all(
            synthetic_summary_after_m5[_c]["position_mae"]
            < synthetic_summary_before_m5[_c]["position_mae"] + 0.005
            for _c in range(1, MAX_OBJECTS + 1)
        )
        and all(
            synthetic_summary_after_m5[_c]["hard_count_accuracy"]
            > synthetic_summary_before_m5[_c]["hard_count_accuracy"] - 0.03
            for _c in range(1, MAX_OBJECTS + 1)
        )
        and all(
            synthetic_summary_after_m5[_c]["count_mae"]
            < synthetic_summary_before_m5[_c]["count_mae"] + 0.03
            for _c in range(1, MAX_OBJECTS + 1)
        )
    )

    mo.md(
        "### Milestone 5 synthetic non-collapse check\n\n"
        f"Synthetic validation NPE before/after SC: `{synthetic_npe_before_m5:.3f}` → `{synthetic_npe_after_m5:.3f}`; delta `{synthetic_npe_delta_m5:.3f}`.\n\n"
        f"Ground-truth latent summary before SC: `{synthetic_summary_before_m5}`\n\n"
        f"Ground-truth latent summary after SC: `{synthetic_summary_after_m5}`\n\n"
        f"Held-out synthetic performance did not collapse: `{synthetic_no_collapse_m5}`."
    )
    return synthetic_no_collapse_m5, synthetic_npe_delta_m5
...
def _(
    ...
    real_estimates_before_m5 = guide_point_estimates(guide_params_m4, real_images_m5)
    real_estimates_after_m5 = guide_point_estimates(guide_params_m5, real_images_m5)
    real_posterior_samples_m5 = sample_guide_latents_m5(
        guide_params_m5, real_images_m5, 12, random.PRNGKey(760)
    )
    real_sampled_counts_m5 = jnp.sum(real_posterior_samples_m5["presence"], axis=-1)
    real_sample_count_summary_m5 = {
        "mean_count_by_patch": np.asarray(jnp.mean(real_sampled_counts_m5, axis=0)).round(3).tolist(),
        "std_count_by_patch": np.asarray(jnp.std(real_sampled_counts_m5, axis=0)).round(3).tolist(),
    }
    real_predicted_counts_before_m5 = jnp.sum(real_estimates_before_m5["presence_probs"], axis=1)
    real_predicted_counts_after_m5 = jnp.sum(real_estimates_after_m5["presence_probs"], axis=1)
    real_distinct_score_m5 = float(
        jnp.mean(jnp.std(real_estimates_after_m5["presence_probs"], axis=0))
        + jnp.mean(jnp.std(real_estimates_after_m5["position"], axis=0))
        + jnp.mean(jnp.std(real_estimates_after_m5["size"], axis=0))
        + jnp.mean(jnp.std(real_estimates_after_m5["composition"], axis=0))
    )
    real_posteriors_distinct_m5 = real_distinct_score_m5 > 0.05
    real_estimates_finite_m5 = bool(
        all(
            jnp.all(jnp.isfinite(value))
            for value in real_estimates_after_m5.values()
        )
    )

    fig_real_overlays_m5, axes_real_overlays_m5 = plt.subplots(2, 4, figsize=(11, 5.5))
    for _ax, _idx in zip(axes_real_overlays_m5.ravel(), range(M5_NUM_REAL_PATCHES)):
        _ax.imshow(np.asarray(real_images_m5[_idx]), cmap="magma", interpolation="nearest")
        _ax.set_title(
            f"patch {_idx}\nE[count] {float(real_predicted_counts_before_m5[_idx]):.2f} → {float(real_predicted_counts_after_m5[_idx]):.2f}",
            fontsize=8,
        )
        _ax.set_xticks([])
        _ax.set_yticks([])
        for _obj in range(MAX_OBJECTS):
            _prob = float(real_estimates_after_m5["presence_probs"][_idx, _obj])
            _pos = np.asarray(real_estimates_after_m5["position"][_idx, _obj])
            _size = float(real_estimates_after_m5["size"][_idx, _obj])
            _comp = np.asarray(real_estimates_after_m5["composition"][_idx, _obj])
            _circle = plt.Circle(
                (_pos[1] * (IMAGE_SHAPE[1] - 1), _pos[0] * (IMAGE_SHAPE[0] - 1)),
                radius=_size * IMAGE_SHAPE[0],
                edgecolor=(0.0, 1.0, 1.0, max(0.15, _prob)),
    ...
    fig_real_overlays_m5.suptitle(
        "Real-patch posterior overlays after conservative SC adaptation\npositions/sizes/compositions remain explicit latents",
        y=1.02,
    )
    fig_real_overlays_m5.tight_layout()
    fig_real_overlays_m5
    return real_estimates_finite_m5, real_posteriors_distinct_m5


@app.cell(hide_code=True)
def _(mo):
    mo.md("""
    ### Milestone 5 real-patch posterior diversity

    "
        f"Predicted expected counts before SC: `{np.asarray(real_predicted_counts_before_m5).round(3).tolist()}`.

    "
        f"Predicted expected counts after SC: `{np.asarray(real_predicted_counts_after_m5).round(3).tolist()}`.

    "
        f"Posterior sampled count summary after SC: `{real_sample_count_summary_m5}`.

    "
        f"Posterior mean distinctness score across real patches: `{real_distinct_score_m5:.3f}`; distinct: `{real_posteriors_distinct_m5}`.

    "
        f"Real posterior estimates finite: `{real_estimates_finite_m5}`.
    """)
    return
...
def _(
    ...
    fixed_eval_sc_before_m5,
    mo,
    real_estimates_finite_m5,
    real_posteriors_distinct_m5,
    sc_fixed_eval_decreased_m5,
    ...
    milestone_5_passed = bool(
        sc_initial_finite_m5
        and sc_fixed_eval_decreased_m5
        and synthetic_no_collapse_m5
        and real_posteriors_distinct_m5
        and real_estimates_finite_m5
    )

    mo.md(
        f"""
        ## Milestone 5 report — Bayesian self-consistency adaptation

        **Implemented.** Loaded unlabelled real patches from `example.jpg`; implemented vectorised `log p_model(theta, x_real)`, `log q_phi(theta | x_real)`, and the Bayesian log-ratio variance loss `Var_l[log p_model(theta_l, x_real) - log q_phi(theta_l | x_real)]`. Latent proposals are sampled from a stop-gradient current guide. The renderer and likelihood are not trained, and no reconstruction MSE or cycle objective is used.

        **Verified.** The SC log-ratio, loss, and guide gradients are finite; guide gradient norm is `{float(sc_initial_grad_norm_m5):.4f}`. A conservative clipped update reduced a fixed-proposal SC monitor from `{fixed_eval_sc_before_m5:.6f}` to `{fixed_eval_sc_after_m5:.6f}`. Held-out synthetic NPE changed by only `{synthetic_npe_delta_m5:.3f}`, and the synthetic ground-truth latent checks did not collapse: `{synthetic_no_collapse_m5}`. Real-patch posterior samples/means differ across patches and remain finite/interpretable.

        **Concerns.** The real image is only weakly calibrated to the simulator by fixed preprocessing, and the model remains restrictive because of x-anchored slots. Larger SC learning rates were observed during development to damage synthetic performance, so this notebook deliberately uses a tiny, conservative adaptation. This is a safe proof-of-mechanism, not a claim that real-domain adaptation is solved.

        **Milestone passed:** `{milestone_5_passed}`.

        **Next.** Only after inspecting the real overlays and synthetic non-collapse diagnostics should Milestone 6 controlled model extensions be attempted, one explicit probabilistic mechanism at a time.
        """
    )
    return
