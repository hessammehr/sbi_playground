...
def _(CHANNELS_V3, COMPOSITION_DIM_V3, jnp, mo, random):
    def init_renderer_params_v3(key, hidden_dim=32, composition_colour_scale=1.2):
        ...
        b2 = jnp.array([-1.0, 0.0, 0.0, 0.0], dtype=jnp.float32)
        return {
            "w1": w1,
            "b1": b1,
            "w2": w2,
            ...
            "delta_scale": jnp.array(0.45, dtype=jnp.float32),
...
def _(
    ...
    def patch_model_v3(image=None, renderer_params=RENDERER_PARAMS_V3, image_shape=IMAGE_SHAPE_V3):
        ...
        observation_noise = numpyro.sample(
            "observation_noise_v3", dist.LogNormal(jnp.log(0.025), 0.30)
        )
        presence = numpyro.sample(
