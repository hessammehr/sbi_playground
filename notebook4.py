...
def _(mo):
    mo.md("""
    # Renderer v3 — Milestone 2: amortised NumPyro guide

    This guide mirrors the v3 model's interpretable latent sites. It may use a CNN encoder, but it remains a NumPyro guide: latent variables are sampled at the corresponding named sites from constrained distributions parameterised by the encoder.

    Renderer parameters are not inferred here. The guide estimates only structured object/background latents: background RGB, observation noise, presence, position, size, and composition.
    """)
    return
...
def _(
    ...
    def positive_v3(raw, floor=1e-3):
        return jnn.softplus(raw) + floor
    ...
    def ensure_image_batch_v3(image):
        image = jnp.asarray(image, dtype=jnp.float32)
        if image.ndim == 3:
            return image[None, ...], True
        if image.ndim == 4:
            return image, False
        raise ValueError(f"v3 image must have shape HxWxC or BxHxWxC, got {image.shape}")


    def conv2d_same_v3(image_batch, kernel, bias, stride=1):
        return (
            jax.lax.conv_general_dilated(
                image_batch,
                kernel,
    ...
    def parse_guide_output_v3(raw_output):
        ...
        return {
            ...
            "noise_loc": noise_raw[:, 0],
            "noise_scale": positive_v3(noise_raw[:, 1], floor=0.02),
            "presence_logits": presence_logits,
            "position_alpha": positive_v3(position_raw[..., 0]),
            "position_beta": positive_v3(position_raw[..., 1]),
            "size_alpha": positive_v3(size_raw[..., 0]),
            "size_beta": positive_v3(size_raw[..., 1]),
            "composition_concentration": positive_v3(composition_raw),
        }
    ...
    def make_guide_distributions_v3(parsed_params):
        ...
        return {
            "background_rgb_v3": dist.TransformedDistribution(
                dist.Beta(parsed_params["background_alpha"], parsed_params["background_beta"]),
                dist.transforms.AffineTransform(bg_low, bg_high - bg_low),
            ).to_event(1),
            "observation_noise_v3": dist.LogNormal(parsed_params["noise_loc"], parsed_params["noise_scale"]),
            "presence_v3": dist.Bernoulli(logits=parsed_params["presence_logits"]).to_event(1),
            "position_v3": dist.TransformedDistribution(
                dist.Beta(parsed_params["position_alpha"], parsed_params["position_beta"]),
                dist.transforms.AffineTransform(POSITION_LOW_V3, POSITION_SCALE_V3),
            ).to_event(2),
            "size_v3": dist.TransformedDistribution(
                dist.Beta(parsed_params["size_alpha"], parsed_params["size_beta"]),
                dist.transforms.AffineTransform(SIZE_LOW_V3, SIZE_HIGH_V3 - SIZE_LOW_V3),
            ).to_event(1),
            "composition_v3": dist.Dirichlet(parsed_params["composition_concentration"]).to_event(1),
        }


    def amortized_guide_v3(image, guide_params):
        image_batch, _ = ensure_image_batch_v3(image)
        distributions = make_guide_distributions_v3(guide_distribution_params_v3(guide_params, image_batch))
        with numpyro.plate("batch_v3", image_batch.shape[0]):
            numpyro.sample("background_rgb_v3", distributions["background_rgb_v3"])
            numpyro.sample("observation_noise_v3", distributions["observation_noise_v3"])
            numpyro.sample("presence_v3", distributions["presence_v3"])
            numpyro.sample("position_v3", distributions["position_v3"])
            numpyro.sample("size_v3", distributions["size_v3"])
            numpyro.sample("composition_v3", distributions["composition_v3"])


    def guide_log_prob_v3(guide_params, image, latents):
        image_batch, was_single = ensure_image_batch_v3(image)
        distributions = make_guide_distributions_v3(guide_distribution_params_v3(guide_params, image_batch))
        terms = []
        for name in LATENT_SITE_NAMES_V3:
            value = jnp.asarray(latents[name])
            if was_single:
                if name in ("observation_noise_v3",) and value.ndim == 0:
                    value = value[None]
                elif name == "background_rgb_v3" and value.ndim == 1:
                    value = value[None, :]
                elif name in ("presence_v3", "size_v3") and value.ndim == 1:
                    value = value[None, :]
                elif name == "position_v3" and value.ndim == 2:
                    value = value[None, :, :]
                elif name == "composition_v3" and value.ndim == 2:
                    value = value[None, :, :]
    ...
    def guide_point_estimates_v3(guide_params, image):
        parsed = guide_distribution_params_v3(guide_params, image)
        bg_low = jnp.array([0.45, 0.45, 0.35], dtype=jnp.float32)
        bg_high = jnp.array([0.88, 0.88, 0.82], dtype=jnp.float32)
        background_unit = parsed["background_alpha"] / (parsed["background_alpha"] + parsed["background_beta"])
        background_rgb = bg_low + (bg_high - bg_low) * background_unit
        ...
        return {
            ...
            "presence_probs_v3": presence_probs,
...
def _(
    ...
    guide_test_images_v3_m2 = prior_predictive_v3["obs_v3"][:5]
    guide_trace_v3_m2 = handlers.trace(
        handlers.seed(lambda image: amortized_guide_v3(image, guide_params_v3_m2), random.PRNGKey(3102))
    ).get_trace(guide_test_images_v3_m2)

    guide_sample_sites_v3_m2 = tuple(name for name, site in guide_trace_v3_m2.items() if site["type"] == "sample")
    guide_latent_sites_v3_m2 = tuple(name for name in guide_sample_sites_v3_m2 if name in LATENT_SITE_NAMES_V3)
    guide_sites_match_v3_m2 = guide_latent_sites_v3_m2 == LATENT_SITE_NAMES_V3
    guide_site_shapes_v3_m2 = {name: tuple(guide_trace_v3_m2[name]["value"].shape) for name in LATENT_SITE_NAMES_V3}
    expected_guide_shapes_v3_m2 = {
        "background_rgb_v3": (5, CHANNELS_V3),
        "observation_noise_v3": (5,),
        "presence_v3": (5, MAX_OBJECTS_V3),
        "position_v3": (5, MAX_OBJECTS_V3, 2),
        "size_v3": (5, MAX_OBJECTS_V3),
        "composition_v3": (5, MAX_OBJECTS_V3, COMPOSITION_DIM_V3),
    }
    guide_shape_checks_v3_m2 = {
        name: guide_site_shapes_v3_m2[name] == expected_guide_shapes_v3_m2[name]
        for name in LATENT_SITE_NAMES_V3
    }

    guide_samples_v3_m2 = {name: guide_trace_v3_m2[name]["value"] for name in LATENT_SITE_NAMES_V3}
    guide_support_checks_v3_m2 = {
        "background_rgb_v3": bool(jnp.all((guide_samples_v3_m2["background_rgb_v3"] >= jnp.array([0.45, 0.45, 0.35])) & (guide_samples_v3_m2["background_rgb_v3"] <= jnp.array([0.88, 0.88, 0.82])))),
        "observation_noise_v3": bool(jnp.all(guide_samples_v3_m2["observation_noise_v3"] > 0.0)),
        "presence_v3": bool(jnp.all((guide_samples_v3_m2["presence_v3"] == 0.0) | (guide_samples_v3_m2["presence_v3"] == 1.0))),
        "position_v3": bool(jnp.all((guide_samples_v3_m2["position_v3"] >= POSITION_LOW_V3) & (guide_samples_v3_m2["position_v3"] <= POSITION_HIGH_V3))),
        "size_v3": bool(jnp.all((guide_samples_v3_m2["size_v3"] >= SIZE_LOW_V3) & (guide_samples_v3_m2["size_v3"] <= SIZE_HIGH_V3))),
        "composition_v3": bool(jnp.all(guide_samples_v3_m2["composition_v3"] > 0.0) & jnp.allclose(jnp.sum(guide_samples_v3_m2["composition_v3"], axis=-1), 1.0, atol=1e-5)),
    }

    guide_shapes_support_ok_v3_m2 = bool(
        guide_sites_match_v3_m2
        and all(guide_shape_checks_v3_m2.values())
        and all(guide_support_checks_v3_m2.values())
    )

    mo.md(
        "### Renderer v3 guide run/support check\n\n"
        f"Guide latent sample sites match model sites: `{guide_sites_match_v3_m2}`.\n\n"
        f"Guide sample shapes: `{guide_site_shapes_v3_m2}`.\n\n"
        f"Guide support checks: `{guide_support_checks_v3_m2}`."
    )
    return guide_shapes_support_ok_v3_m2, guide_test_images_v3_m2
...
def _(
    ...
    guide_params_v3_m2,
    guide_point_estimates_v3,
    guide_test_images_v3_m2,
    jnp,
    mo,
    np,
    prior_predictive_v3,
):
    guide_prior_latents_v3_m2 = {name: prior_predictive_v3[name][:5] for name in LATENT_SITE_NAMES_V3}
    guide_log_prob_prior_v3_m2 = guide_log_prob_v3(
        guide_params_v3_m2, guide_test_images_v3_m2, guide_prior_latents_v3_m2
    )
    guide_log_prob_finite_v3_m2 = bool(jnp.all(jnp.isfinite(guide_log_prob_prior_v3_m2)))

    guide_estimates_pair_v3_m2 = guide_point_estimates_v3(guide_params_v3_m2, guide_test_images_v3_m2[:2])
    guide_image_dependence_score_v3_m2 = float(
        jnp.sum(jnp.abs(guide_estimates_pair_v3_m2["presence_probs_v3"][0] - guide_estimates_pair_v3_m2["presence_probs_v3"][1]))
        + jnp.sum(jnp.abs(guide_estimates_pair_v3_m2["position_v3"][0] - guide_estimates_pair_v3_m2["position_v3"][1]))
        + jnp.sum(jnp.abs(guide_estimates_pair_v3_m2["size_v3"][0] - guide_estimates_pair_v3_m2["size_v3"][1]))
        + jnp.sum(jnp.abs(guide_estimates_pair_v3_m2["composition_v3"][0] - guide_estimates_pair_v3_m2["composition_v3"][1]))
    )
    guide_output_changes_with_image_v3_m2 = guide_image_dependence_score_v3_m2 > 1e-4

    mo.md(
        "### Renderer v3 guide log-q and image-dependence checks\n\n"
        f"`log q_phi(theta | image)` for prior-sampled latents: `{np.asarray(guide_log_prob_prior_v3_m2).round(3).tolist()}`.\n\n"
        f"All finite: `{guide_log_prob_finite_v3_m2}`.\n\n"
        f"Posterior-parameter image-dependence score between two images: `{guide_image_dependence_score_v3_m2:.4f}`."
    )
    return guide_log_prob_finite_v3_m2, guide_output_changes_with_image_v3_m2
...
def _(
    ...
    guide_vis_estimates_v3_m2 = guide_point_estimates_v3(guide_params_v3_m2, guide_test_images_v3_m2[:4])
    fig_guide_initial_v3_m2, axes_guide_initial_v3_m2 = plt.subplots(1, 4, figsize=(11, 3))
    for _ax, _idx in zip(axes_guide_initial_v3_m2, range(4)):
        _ax.imshow(np.asarray(guide_test_images_v3_m2[_idx]).clip(0.0, 1.0), interpolation="nearest")
        _ax.set_title(f"untrained guide, image {_idx}", fontsize=8)
        _ax.set_xticks([])
        _ax.set_yticks([])
        for _obj in range(MAX_OBJECTS_V3):
            _prob = float(guide_vis_estimates_v3_m2["presence_probs_v3"][_idx, _obj])
            if _prob > 0.20:
                _pos = np.asarray(guide_vis_estimates_v3_m2["position_v3"][_idx, _obj])
                _size = float(guide_vis_estimates_v3_m2["size_v3"][_idx, _obj])
                _comp = np.asarray(guide_vis_estimates_v3_m2["composition_v3"][_idx, _obj])
                _colour = np.asarray(composition_to_rgb_v3(jnp.asarray(_comp)))
                _ax.add_patch(
                    plt.Circle(
                        (_pos[1] * (IMAGE_SHAPE_V3[1] - 1), _pos[0] * (IMAGE_SHAPE_V3[0] - 1)),
                        radius=_size * IMAGE_SHAPE_V3[0],
                        edgecolor=(*np.clip(_colour, 0.0, 1.0), max(0.15, _prob)),
                        facecolor="none",
                        linewidth=0.8,
                    )
                )
    fig_guide_initial_v3_m2.suptitle("Untrained v3 guide posterior means (diagnostic only)", y=1.02)
    fig_guide_initial_v3_m2.tight_layout()
    fig_guide_initial_v3_m2
    return


@app.cell(hide_code=True)
def _(
    guide_log_prob_finite_v3_m2,
    guide_output_changes_with_image_v3_m2,
    guide_shapes_support_ok_v3_m2,
    mo,
):
    milestone_v3_guide_passed = bool(
        guide_shapes_support_ok_v3_m2
        and guide_log_prob_finite_v3_m2
        and guide_output_changes_with_image_v3_m2
    )

    mo.md(
        f"""
        ## Renderer v3 Milestone 2 report — amortised guide

        **Implemented.** A NumPyro guide `amortized_guide_v3(image, guide_params)` with mirrored sample sites `background_rgb_v3`, `observation_noise_v3`, `presence_v3`, `position_v3`, `size_v3`, and `composition_v3`. A CNN encoder parameterises constrained distributions for each site.

        **Verified.** The guide runs on a batch of RGB images, sample sites and shapes match the model, sampled values satisfy supports, externally supplied prior latents have finite `log q_phi(theta | image)`, and guide outputs change with image input. The visual overlay is explicitly untrained and diagnostic only.

        **Concerns.** The guide has not been trained. The next milestone must include labelled synthetic training plus train-set recovery / tiny-overfit checks before any held-out or pseudo-real claims.

        **Renderer v3 Milestone 2 passed:** `{milestone_v3_guide_passed}`.
        """
    )
    return
...
def _(mo):
    mo.md("""
    # Renderer v3 — Milestone 3: synthetic NPE training + train-set sanity

    The guide is trained on labelled synthetic pairs from `patch_model_v3` with the NPE objective

    \[
    L_\mathrm{NPE} = -\log q_\phi(	heta_\mathrm{sim} \mid x_\mathrm{sim}).
    \]

    This milestone includes the requested **4.5 train-set sanity check**: evaluate on the synthetic training images, plus a separate tiny-subset overfit test. Held-out metrics are interpreted only after the training-set checks.
    """)
    return
...
def _(LATENT_SITE_NAMES_V3, Predictive, mo, patch_model_v3, random):
    ...
    mo.md(
        "### v3 synthetic data\n\n"
        f"Generated `{M3_V3_TRAIN_SIZE}` training pairs and `{M3_V3_VAL_SIZE}` held-out validation pairs.\n\n"
...
@app.cell
def _(LATENT_SITE_NAMES_V3, guide_log_prob_v3, jax, jnp, optax):
    def select_batch_v3_m3(data, indices):
        return {name: value[indices] for name, value in data.items()}


    def npe_loss_v3_m3(guide_params, batch):
        latents = {name: batch[name] for name in LATENT_SITE_NAMES_V3}
        return -jnp.mean(guide_log_prob_v3(guide_params, batch["obs_v3"], latents))


    optimizer_v3_m3 = optax.adam(6e-4)


    @jax.jit
    def npe_train_step_v3_m3(guide_params, opt_state, batch):
        loss_value, gradients = jax.value_and_grad(npe_loss_v3_m3)(guide_params, batch)
        updates, opt_state = optimizer_v3_m3.update(gradients, opt_state, guide_params)
        guide_params = optax.apply_updates(guide_params, updates)
        return guide_params, opt_state, loss_value

    return (
        npe_loss_v3_m3,
        npe_train_step_v3_m3,
        optimizer_v3_m3,
        select_batch_v3_m3,
    )
...
def _(
    ...
    initial_train_loss_v3_m3 = float(
        npe_loss_v3_m3(
            guide_params_current_v3_m3,
            select_batch_v3_m3(synthetic_train_v3_m3, jnp.arange(M3_V3_BATCH_SIZE)),
        )
    )
    initial_val_loss_v3_m3 = float(npe_loss_v3_m3(guide_params_current_v3_m3, synthetic_val_v3_m3))
    best_val_loss_v3_m3 = initial_val_loss_v3_m3
    guide_params_best_v3_m3 = guide_params_current_v3_m3
    best_step_v3_m3 = 0
    ...
    for step_v3_m3 in tqdm(range(1, M3_V3_NUM_STEPS + 1), miniters=100):
        ...
        batch_indices_v3_m3 = random.choice(
            subkey_v3_m3,
            M3_V3_TRAIN_SIZE,
            shape=(M3_V3_BATCH_SIZE,),
            replace=False,
        )
        batch_v3_m3 = select_batch_v3_m3(synthetic_train_v3_m3, batch_indices_v3_m3)
        guide_params_current_v3_m3, opt_state_v3_m3, train_loss_v3_m3 = npe_train_step_v3_m3(
            guide_params_current_v3_m3, opt_state_v3_m3, batch_v3_m3
        )
        if step_v3_m3 % M3_V3_EVAL_EVERY == 0 or step_v3_m3 == M3_V3_NUM_STEPS:
            val_loss_v3_m3 = float(npe_loss_v3_m3(guide_params_current_v3_m3, synthetic_val_v3_m3))
            training_history_v3_m3.append((step_v3_m3, float(train_loss_v3_m3), val_loss_v3_m3))
            if val_loss_v3_m3 < best_val_loss_v3_m3:
                best_val_loss_v3_m3 = val_loss_v3_m3
                guide_params_best_v3_m3 = guide_params_current_v3_m3
                best_step_v3_m3 = step_v3_m3

    guide_params_v3_m3 = guide_params_best_v3_m3
    final_val_loss_v3_m3 = float(npe_loss_v3_m3(guide_params_v3_m3, synthetic_val_v3_m3))
    loss_decreased_v3_m3 = final_val_loss_v3_m3 < initial_val_loss_v3_m3

    mo.md(
        "### v3 NPE training run\n\n"
        f"Initial validation NPE loss: `{initial_val_loss_v3_m3:.3f}`.\n\n"
        f"Best validation NPE loss: `{best_val_loss_v3_m3:.3f}` at step `{best_step_v3_m3}`.\n\n"
        f"Loss decreased: `{loss_decreased_v3_m3}`."
    )
    return (
        best_val_loss_v3_m3,
        guide_params_v3_m3,
        initial_val_loss_v3_m3,
        loss_decreased_v3_m3,
        training_history_v3_m3,
    )


@app.cell
def _(np, plt, training_history_v3_m3):
    history_array_v3_m3 = np.asarray(training_history_v3_m3)
    fig_loss_v3_m3, ax_loss_v3_m3 = plt.subplots(figsize=(6, 3.5))
    ax_loss_v3_m3.plot(history_array_v3_m3[:, 0], history_array_v3_m3[:, 1], marker="o", label="mini-batch train")
    ax_loss_v3_m3.plot(history_array_v3_m3[:, 0], history_array_v3_m3[:, 2], marker="o", label="held-out validation")
    ax_loss_v3_m3.set_xlabel("training step")
    ax_loss_v3_m3.set_ylabel("NPE loss = - log q")
    ax_loss_v3_m3.set_title("Renderer v3 synthetic NPE training")
    ax_loss_v3_m3.grid(alpha=0.25)
    ax_loss_v3_m3.legend()
    fig_loss_v3_m3.tight_layout()
    fig_loss_v3_m3
    return
...
def _(
    ...
    M3_V3_TRAIN_EVAL_N = 512
    M3_V3_VAL_EVAL_N = 512


    def latent_metrics_v3_m3(guide_params, data, n_eval):
        eval_data = {name: value[:n_eval] for name, value in data.items()}
        estimates = guide_point_estimates_v3(guide_params, eval_data["obs_v3"])
        presence = eval_data["presence_v3"]
        active_count = jnp.sum(presence)

        position_abs = jnp.sum(
            jnp.abs(estimates["position_v3"] - eval_data["position_v3"]) * presence[:, :, None]
        )
        size_abs = jnp.sum(jnp.abs(estimates["size_v3"] - eval_data["size_v3"]) * presence)
        composition_abs = jnp.sum(
            jnp.abs(estimates["composition_v3"] - eval_data["composition_v3"]) * presence[:, :, None]
        )
        presence_mae = jnp.mean(jnp.abs(estimates["presence_probs_v3"] - presence))
        expected_count = jnp.sum(estimates["presence_probs_v3"], axis=1)
        count_mae = jnp.mean(jnp.abs(expected_count - eval_data["count_v3"]))
        hard_count = jnp.sum((estimates["presence_probs_v3"] > 0.5).astype(jnp.float32), axis=1)
        hard_count_accuracy = jnp.mean(hard_count == eval_data["count_v3"])

        prior_position_mean = jnp.broadcast_to(POSITION_CENTER_V3, eval_data["position_v3"].shape)
        prior_size_mean = ((SIZE_LOW_V3 + SIZE_HIGH_V3) / 2.0) * jnp.ones_like(eval_data["size_v3"])
        prior_composition_mean = (1.0 / COMPOSITION_DIM_V3) * jnp.ones_like(eval_data["composition_v3"])
        prior_presence_probs = 0.35 * jnp.ones_like(presence)
        prior_expected_count = jnp.sum(prior_presence_probs, axis=1)

        baseline = {
            "position_mae": float(
                jnp.sum(jnp.abs(prior_position_mean - eval_data["position_v3"]) * presence[:, :, None])
                / (2.0 * active_count + 1e-6)
            ),
            "size_mae": float(
                jnp.sum(jnp.abs(prior_size_mean - eval_data["size_v3"]) * presence)
                / (active_count + 1e-6)
            ),
            "composition_mae": float(
                jnp.sum(jnp.abs(prior_composition_mean - eval_data["composition_v3"]) * presence[:, :, None])
                / (COMPOSITION_DIM_V3 * active_count + 1e-6)
            ),
            "presence_mae": float(jnp.mean(jnp.abs(prior_presence_probs - presence))),
            "count_mae": float(jnp.mean(jnp.abs(prior_expected_count - eval_data["count_v3"]))),
        }
        metrics = {
            "position_mae": float(position_abs / (2.0 * active_count + 1e-6)),
            "size_mae": float(size_abs / (active_count + 1e-6)),
            "composition_mae": float(composition_abs / (COMPOSITION_DIM_V3 * active_count + 1e-6)),
            "presence_mae": float(presence_mae),
            "count_mae": float(count_mae),
            "hard_count_accuracy": float(hard_count_accuracy),
            "mean_expected_count": float(jnp.mean(expected_count)),
            "mean_true_count": float(jnp.mean(eval_data["count_v3"])),
        }
        return metrics, baseline, estimates, eval_data


    train_metrics_v3_m3, train_baseline_v3_m3, train_estimates_v3_m3, train_eval_data_v3_m3 = latent_metrics_v3_m3(
        guide_params_v3_m3, synthetic_train_v3_m3, M3_V3_TRAIN_EVAL_N
    )
    val_metrics_v3_m3, val_baseline_v3_m3, val_estimates_v3_m3, val_eval_data_v3_m3 = latent_metrics_v3_m3(
        guide_params_v3_m3, synthetic_val_v3_m3, M3_V3_VAL_EVAL_N
    )

    train_set_sanity_passed_v3_m3 = bool(
        train_metrics_v3_m3["position_mae"] < 0.04
        and train_metrics_v3_m3["composition_mae"] < 0.16
        and train_metrics_v3_m3["presence_mae"] < 0.16
        and train_metrics_v3_m3["count_mae"] < 1.0
        and train_metrics_v3_m3["position_mae"] < train_baseline_v3_m3["position_mae"]
        and train_metrics_v3_m3["composition_mae"] < train_baseline_v3_m3["composition_mae"]
        and train_metrics_v3_m3["presence_mae"] < train_baseline_v3_m3["presence_mae"]
        and train_metrics_v3_m3["count_mae"] < train_baseline_v3_m3["count_mae"]
    )
    heldout_sanity_passed_v3_m3 = bool(
        val_metrics_v3_m3["position_mae"] < 0.045
        and val_metrics_v3_m3["composition_mae"] < 0.17
        and val_metrics_v3_m3["presence_mae"] < 0.20
        and val_metrics_v3_m3["count_mae"] < 1.2
        and val_metrics_v3_m3["position_mae"] < val_baseline_v3_m3["position_mae"]
        and val_metrics_v3_m3["composition_mae"] < val_baseline_v3_m3["composition_mae"]
        and val_metrics_v3_m3["presence_mae"] < val_baseline_v3_m3["presence_mae"]
        and val_metrics_v3_m3["count_mae"] < val_baseline_v3_m3["count_mae"]
    )
    size_recovery_concern_v3_m3 = bool(
        train_metrics_v3_m3["size_mae"] >= train_baseline_v3_m3["size_mae"]
        or val_metrics_v3_m3["size_mae"] >= val_baseline_v3_m3["size_mae"]
    )


    def active_colour_summary_v3_m3(estimates, eval_data):
        true_colours = composition_to_rgb_v3(eval_data["composition_v3"])
        pred_colours = composition_to_rgb_v3(estimates["composition_v3"])
        active = eval_data["presence_v3"].astype(bool)
        active_weight = eval_data["presence_v3"][:, :, None]
        active_count = jnp.sum(eval_data["presence_v3"])
        return {
            "true_mean": np.asarray(jnp.mean(true_colours[active], axis=0)).round(3).tolist(),
            "true_std": np.asarray(jnp.std(true_colours[active], axis=0)).round(3).tolist(),
            "pred_mean": np.asarray(jnp.mean(pred_colours[active], axis=0)).round(3).tolist(),
            "pred_std": np.asarray(jnp.std(pred_colours[active], axis=0)).round(3).tolist(),
            "active_colour_mae": float(jnp.sum(jnp.abs(true_colours - pred_colours) * active_weight) / (3.0 * active_count + 1e-6)),
            "std_ratio_mean": float(jnp.mean(jnp.std(pred_colours[active], axis=0) / (jnp.std(true_colours[active], axis=0) + 1e-6))),
        }


    train_colour_summary_v3_m3 = active_colour_summary_v3_m3(train_estimates_v3_m3, train_eval_data_v3_m3)
    val_colour_summary_v3_m3 = active_colour_summary_v3_m3(val_estimates_v3_m3, val_eval_data_v3_m3)
    colour_recovery_passed_v3_m3 = bool(
        train_colour_summary_v3_m3["std_ratio_mean"] > 0.55
        and val_colour_summary_v3_m3["std_ratio_mean"] > 0.50
        and train_colour_summary_v3_m3["active_colour_mae"] < 0.07
        and val_colour_summary_v3_m3["active_colour_mae"] < 0.08
    )

    mo.md(
        "### v3 train-set and held-out latent recovery\n\n"
        "Metrics use the known synthetic ground-truth latents. Slots are anchored, so direct slotwise errors are used; no image reconstruction metric is used as the training gate.\n\n"
        f"Training metrics: `{train_metrics_v3_m3}`\n\n"
        f"Training prior baselines: `{train_baseline_v3_m3}`\n\n"
        f"Held-out metrics: `{val_metrics_v3_m3}`\n\n"
        f"Held-out prior baselines: `{val_baseline_v3_m3}`\n\n"
        f"Train-set sanity passed: `{train_set_sanity_passed_v3_m3}`. Held-out sanity passed: `{heldout_sanity_passed_v3_m3}`.\n\n"
        f"Size recovery concern (size MAE not clearly better than prior mean): `{size_recovery_concern_v3_m3}`.\n\n"
        f"Colour recovery summaries: train `{train_colour_summary_v3_m3}`, held-out `{val_colour_summary_v3_m3}`. Colour recovery passed: `{colour_recovery_passed_v3_m3}`."
    )
    return (
        colour_recovery_passed_v3_m3,
        heldout_sanity_passed_v3_m3,
        latent_metrics_v3_m3,
        size_recovery_concern_v3_m3,
...
def _(
    ...
    def render_point_estimates_v3_m3(estimates, n_images):
        return jax.vmap(render_scene_v3)(
            estimates["background_rgb_v3"][:n_images],
            estimates["presence_probs_v3"][:n_images],
            estimates["position_v3"][:n_images],
            estimates["size_v3"][:n_images],
            estimates["composition_v3"][:n_images],
        )


    train_render_estimates_v3_m3 = render_point_estimates_v3_m3(train_estimates_v3_m3, 6)
    val_render_estimates_v3_m3 = render_point_estimates_v3_m3(val_estimates_v3_m3, 6)

    fig_train_examples_v3_m3, axes_train_examples_v3_m3 = plt.subplots(6, 3, figsize=(8, 13))
    for _row in range(6):
        _obs = np.asarray(train_eval_data_v3_m3["obs_v3"][_row]).clip(0.0, 1.0)
        _true = np.asarray(train_eval_data_v3_m3["mean_v3"][_row]).clip(0.0, 1.0)
        _pred = np.asarray(train_render_estimates_v3_m3[_row]).clip(0.0, 1.0)
        for _col, (_image, _title) in enumerate(zip([_obs, _true, _pred], ["train obs", "true mean", "guide point render"])):
            axes_train_examples_v3_m3[_row, _col].imshow(_image, interpolation="nearest")
            axes_train_examples_v3_m3[_row, _col].set_title(_title if _row == 0 else "", fontsize=9)
            axes_train_examples_v3_m3[_row, _col].set_xticks([])
            axes_train_examples_v3_m3[_row, _col].set_yticks([])
        for _obj in range(MAX_OBJECTS_V3):
            if float(train_eval_data_v3_m3["presence_v3"][_row, _obj]) > 0.5:
                _pos = np.asarray(train_eval_data_v3_m3["position_v3"][_row, _obj])
                _size = float(train_eval_data_v3_m3["size_v3"][_row, _obj])
                axes_train_examples_v3_m3[_row, 0].add_patch(
                    plt.Circle((_pos[1] * 63, _pos[0] * 63), radius=_size * 64, edgecolor="cyan", facecolor="none", linewidth=1.0)
                )
            _prob = float(train_estimates_v3_m3["presence_probs_v3"][_row, _obj])
            if _prob > 0.35:
                _pos = np.asarray(train_estimates_v3_m3["position_v3"][_row, _obj])
                _size = float(train_estimates_v3_m3["size_v3"][_row, _obj])
                axes_train_examples_v3_m3[_row, 2].add_patch(
                    plt.Circle((_pos[1] * 63, _pos[0] * 63), radius=_size * 64, edgecolor="magenta", facecolor="none", linewidth=0.8)
                )
    fig_train_examples_v3_m3.suptitle("v3 training-set sanity examples: true objects cyan, inferred slots magenta", y=0.995)
    fig_train_examples_v3_m3.tight_layout()
    fig_train_examples_v3_m3
    return (val_render_estimates_v3_m3,)
...
def _(
    ...
    val_estimates_v3_m3,
    val_eval_data_v3_m3,
    val_render_estimates_v3_m3,
):
    fig_val_examples_v3_m3, axes_val_examples_v3_m3 = plt.subplots(6, 3, figsize=(8, 13))
    for _row in range(6):
        _obs = np.asarray(val_eval_data_v3_m3["obs_v3"][_row]).clip(0.0, 1.0)
        _true = np.asarray(val_eval_data_v3_m3["mean_v3"][_row]).clip(0.0, 1.0)
        _pred = np.asarray(val_render_estimates_v3_m3[_row]).clip(0.0, 1.0)
        for _col, (_image, _title) in enumerate(zip([_obs, _true, _pred], ["held-out obs", "true mean", "guide point render"])):
            axes_val_examples_v3_m3[_row, _col].imshow(_image, interpolation="nearest")
            axes_val_examples_v3_m3[_row, _col].set_title(_title if _row == 0 else "", fontsize=9)
            axes_val_examples_v3_m3[_row, _col].set_xticks([])
            axes_val_examples_v3_m3[_row, _col].set_yticks([])
        for _obj in range(MAX_OBJECTS_V3):
            if float(val_eval_data_v3_m3["presence_v3"][_row, _obj]) > 0.5:
                _pos = np.asarray(val_eval_data_v3_m3["position_v3"][_row, _obj])
                _size = float(val_eval_data_v3_m3["size_v3"][_row, _obj])
                axes_val_examples_v3_m3[_row, 0].add_patch(
                    plt.Circle((_pos[1] * 63, _pos[0] * 63), radius=_size * 64, edgecolor="cyan", facecolor="none", linewidth=1.0)
                )
            _prob = float(val_estimates_v3_m3["presence_probs_v3"][_row, _obj])
            if _prob > 0.35:
                _pos = np.asarray(val_estimates_v3_m3["position_v3"][_row, _obj])
                _size = float(val_estimates_v3_m3["size_v3"][_row, _obj])
                axes_val_examples_v3_m3[_row, 2].add_patch(
                    plt.Circle((_pos[1] * 63, _pos[0] * 63), radius=_size * 64, edgecolor="magenta", facecolor="none", linewidth=0.8)
                )
    fig_val_examples_v3_m3.suptitle("v3 held-out examples: true objects cyan, inferred slots magenta", y=0.995)
    fig_val_examples_v3_m3.tight_layout()
    fig_val_examples_v3_m3
    return
...
def _(
    ...
    def tiny_overfit_loss_v3_m3(guide_params):
        return -jnp.mean(
            guide_log_prob_v3(
                guide_params,
                tiny_overfit_data_v3_m3["obs_v3"],
                {name: tiny_overfit_data_v3_m3[name] for name in LATENT_SITE_NAMES_V3},
            )
        )


    tiny_optimizer_v3_m3 = optax.chain(optax.clip_by_global_norm(20.0), optax.adam(5e-4))


    @jax.jit
    def tiny_overfit_step_v3_m3(guide_params, opt_state):
        loss_value, gradients = jax.value_and_grad(tiny_overfit_loss_v3_m3)(guide_params)
        updates, opt_state = tiny_optimizer_v3_m3.update(gradients, opt_state, guide_params)
        guide_params = optax.apply_updates(guide_params, updates)
        return guide_params, opt_state, loss_value
    ...
    tiny_best_params_v3_m3 = tiny_params_current_v3_m3
    tiny_best_step_v3_m3 = 0
    tiny_history_v3_m3 = [(0, tiny_initial_loss_v3_m3)]

    for _tiny_step in range(1, M3_V3_TINY_OVERFIT_STEPS + 1):
        tiny_params_current_v3_m3, tiny_opt_state_v3_m3, tiny_loss_value_v3_m3 = tiny_overfit_step_v3_m3(
            tiny_params_current_v3_m3, tiny_opt_state_v3_m3
        )
        _loss_float = float(tiny_loss_value_v3_m3)
        if _loss_float < tiny_best_loss_v3_m3:
            tiny_best_loss_v3_m3 = _loss_float
            tiny_best_params_v3_m3 = tiny_params_current_v3_m3
            tiny_best_step_v3_m3 = _tiny_step
        if _tiny_step % 100 == 0 or _tiny_step == M3_V3_TINY_OVERFIT_STEPS:
            tiny_history_v3_m3.append((_tiny_step, _loss_float))

    guide_params_tiny_overfit_v3_m3 = tiny_best_params_v3_m3
    tiny_final_loss_v3_m3 = float(tiny_overfit_loss_v3_m3(guide_params_tiny_overfit_v3_m3))
    ...
    mo.md(
        "### v3 tiny-subset overfit sanity check\n\n"
        f"Tiny initial loss: `{tiny_initial_loss_v3_m3:.3f}`. Best/final chosen loss: `{tiny_final_loss_v3_m3:.3f}` at step `{tiny_best_step_v3_m3}`.\n\n"
        f"Tiny metrics: `{tiny_metrics_v3_m3}`\n\n"
        f"Tiny overfit passed: `{tiny_overfit_passed_v3_m3}`."
    )
    return (tiny_overfit_passed_v3_m3,)
...
def _(
    ...
    mo,
    size_recovery_concern_v3_m3,
    tiny_overfit_passed_v3_m3,
    train_colour_summary_v3_m3,
    train_set_sanity_passed_v3_m3,
    val_colour_summary_v3_m3,
):
    milestone_v3_synthetic_training_passed = bool(
        loss_decreased_v3_m3
        and train_set_sanity_passed_v3_m3
        and heldout_sanity_passed_v3_m3
        and colour_recovery_passed_v3_m3
        and tiny_overfit_passed_v3_m3
    )

    mo.md(
        f"""
        ## Renderer v3 Milestone 3 report — synthetic training

        **Implemented.** Synthetic NPE training for the v3 guide using `L_NPE = -log q_phi(theta_sim | x_sim)`. The trained parameters are stored as `guide_params_v3_m3`. Added the requested train-set sanity evaluation and a separate tiny-subset overfit check.

        **Verified.** Validation NPE decreased from `{initial_val_loss_v3_m3:.3f}` to `{best_val_loss_v3_m3:.3f}`. Training-set latent recovery passed the sanity gate: `{train_set_sanity_passed_v3_m3}`. Held-out latent recovery passed the current sanity gate: `{heldout_sanity_passed_v3_m3}`. Tiny overfit passed: `{tiny_overfit_passed_v3_m3}`.

        **Numerical concerns.** The visual panels show a serious colour/composition failure: inferred objects collapse toward a narrow yellowish colour range. The explicit colour diagnostic agrees: `colour_recovery_passed_v3_m3 = {colour_recovery_passed_v3_m3}` with summaries train `{train_colour_summary_v3_m3}` and held-out `{val_colour_summary_v3_m3}`. Size recovery is also only marginal (`size_recovery_concern_v3_m3 = {size_recovery_concern_v3_m3}`), though the tiny overfit test can recover size. This milestone is not allowed to pass unless colour/composition semantics are recovered, even if scalar composition MAE or NPE loss looks acceptable.

        **Visual concerns.** Point-render panels are diagnostic only; the objective is latent recovery, not reconstruction MSE. Inspect the train and held-out panels above for missed/extra slots.

        **Renderer v3 Milestone 3 passed:** `{milestone_v3_synthetic_training_passed}`.
